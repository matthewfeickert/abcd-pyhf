...
def test_fixed_poi_fit():
    ...
    assert math.isclose(
        fixed_poi_fit[par_names.index('systematic_uncertainty')][0],
        0,
        abs_tol=1e-1
    )
    assert math.isclose(
        fixed_poi_fit[par_names.index('mu_b')][0],
        observed_yields['A'],
        rel_tol=1e-2
    )
    assert math.isclose(
...
def test_bkg_only_fit():
    ...
    assert math.isclose(
        bkg_only_fit[par_names.index('mu_b')][0],
        observed_yields['A'],
        rel_tol=1e-2
    )
    assert math.isclose(
...
def test_fit():
    ...
    assert math.isclose(
        fit[par_names.index('systematic_uncertainty')][0],
        0,
        abs_tol=1e-1
    )
    assert math.isclose(
        fit[par_names.index('mu_b')][0],
        observed_yields['A'],
        rel_tol=1e-2
    )
    assert math.isclose(
        fit[par_names.index('tau_B')][0],
        observed_yields['B'] / observed_yields['A'],
        rel_tol=1e-2
    )
    assert math.isclose(
        fit[par_names.index('tau_C')][0],
        observed_yields['C'] / observed_yields['A'],
        rel_tol=1e-2
    )
